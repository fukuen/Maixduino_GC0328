...
Maixduino_GC0328::Maixduino_GC0328(uint16_t width, uint16_t height, pixformat_t pixFormat)
...
Maixduino_GC0328::~Maixduino_GC0328()
{
    end();
}

bool Maixduino_GC0328::begin()
{
    if(_dataBuffer)
        free(_dataBuffer);
    if(_aiBuffer)
...
bool Maixduino_GC0328::setPixFormat(pixformat_t pixFormat)
{
...
bool Maixduino_GC0328::setFrameSize(framesize_t frameSize)
{
...
bool Maixduino_GC0328::run(bool run)
{
	if(run)
	{
 ...
 */
uint8_t* Maixduino_GC0328::snapshot()
{
...
void Maixduino_GC0328::setInvert(bool invert)
{
    //FIXME
...
int Maixduino_GC0328::dvpInit(uint32_t freq)
{
...
/*
int Maixduino_GC0328::cambus_scan()
{
...
int Maixduino_GC0328::cambus_read_id(uint8_t addr,uint16_t *manuf_id, uint16_t *device_id)
{
...
int Maixduino_GC0328::cambus_scan_gc0328(void)
{
...
int Maixduino_GC0328::cambus_readb(uint8_t slv_addr, uint8_t reg_addr, uint8_t *reg_data)
{
...
int Maixduino_GC0328::cambus_writeb(uint8_t slv_addr, uint8_t reg_addr, uint8_t reg_data)
{
...
int Maixduino_GC0328::cambus_readw(uint8_t slv_addr, uint8_t reg_addr, uint16_t *reg_data)
{
    return 0;
}

int Maixduino_GC0328::cambus_writew(uint8_t slv_addr, uint8_t reg_addr, uint16_t reg_data)
{
    return 0;
}

int Maixduino_GC0328::cambus_readw2(uint8_t slv_addr, uint16_t reg_addr, uint16_t *reg_data)
{
    return 0;
}

int Maixduino_GC0328::cambus_writew2(uint8_t slv_addr, uint16_t reg_addr, uint16_t reg_data)
{
    return 0;
}
*/

int Maixduino_GC0328::sensor_ov_detect()
{
    /* Reset the sensor */
    DCMI_RESET_HIGH();
    msleep(10);
...
int Maixduino_GC0328::sensro_gc_detect()
{
...
int Maixduino_GC0328::dvpInitIrq()
{
	dvp_config_interrupt(DVP_CFG_START_INT_ENABLE | DVP_CFG_FINISH_INT_ENABLE, 0);
...
int Maixduino_GC0328::ov2640_reset()
{
    int i=0;
...
int Maixduino_GC0328::gc0328_reset()
{
    uint16_t index = 0;
...
int Maixduino_GC0328::ov2640_read_reg(uint8_t reg_addr)
{
    uint8_t reg_data;
...
int Maixduino_GC0328::ov2640_write_reg(uint8_t reg_addr, uint8_t reg_data)
{
    return cambus_writeb(_slaveAddr, reg_addr, reg_data);
}


int Maixduino_GC0328::ov2640_set_pixformat(pixformat_t pixformat)
{
    int i=0;
...
int Maixduino_GC0328::ov2640_set_framesize(framesize_t framesize)
{
    int ret=0;
    uint8_t clkrc;
    uint16_t w = _width;
...
int Maixduino_GC0328::ov2640_set_framerate(framerate_t framerate)
{
    return 0;
}

int Maixduino_GC0328::ov2640_set_contrast(int level)
{
    int ret=0;
...
int Maixduino_GC0328::ov2640_set_brightness(int level)
{
    int ret=0;
...
int Maixduino_GC0328::ov2640_set_saturation(int level)
{
    int ret=0;
...
int Maixduino_GC0328::ov2640_set_gainceiling( gainceiling_t gainceiling)
{
    int ret =0;
...
int Maixduino_GC0328::ov2640_set_quality(int qs)
{
    int ret=0;
...
int Maixduino_GC0328::ov2640_set_colorbar(int enable)
{
    uint8_t reg;
    /* Switch to SENSOR register bank */
...
int Maixduino_GC0328::ov2640_set_auto_gain(int enable, float gain_db, float gain_db_ceiling)
{
   uint8_t reg;
...
int Maixduino_GC0328::ov2640_get_gain_db(float *gain_db)
{
    uint8_t reg, gain;
    int ret = cambus_readb(_slaveAddr, BANK_SEL, &reg);
    ret |= cambus_writeb(_slaveAddr, BANK_SEL, reg | BANK_SEL_SENSOR);
...
int Maixduino_GC0328::ov2640_set_auto_exposure(int enable, int exposure_us)
{
    uint8_t reg;
    int ret = cambus_readb(_slaveAddr, BANK_SEL, &reg);
    ret |= cambus_writeb(_slaveAddr, BANK_SEL, reg | BANK_SEL_SENSOR);
...
int Maixduino_GC0328::ov2640_get_exposure_us(int *exposure_us)
{
...
int Maixduino_GC0328::ov2640_set_auto_whitebal(int enable, float r_gain_db, float g_gain_db, float b_gain_db)
{
    uint8_t reg;
    int ret = cambus_readb(_slaveAddr, BANK_SEL, &reg);
    ret |= cambus_writeb(_slaveAddr, BANK_SEL, reg & (~BANK_SEL_SENSOR));
...
int Maixduino_GC0328::ov2640_get_rgb_gain_db(float *r_gain_db, float *g_gain_db, float *b_gain_db)
{
    uint8_t reg;
    int ret = cambus_readb(_slaveAddr, BANK_SEL, &reg);
    ret |= cambus_writeb(_slaveAddr, BANK_SEL, reg & (~BANK_SEL_SENSOR));
...
int Maixduino_GC0328::ov2640_set_hmirror(int enable)
{
    uint8_t reg;
    int ret = cambus_readb(_slaveAddr, BANK_SEL, &reg);
    ret |= cambus_writeb(_slaveAddr, BANK_SEL, reg | BANK_SEL_SENSOR);
...
int Maixduino_GC0328::ov2640_set_vflip(int enable)
{
    uint8_t reg;
    int ret = cambus_readb(_slaveAddr, BANK_SEL, &reg);
    ret |= cambus_writeb(_slaveAddr, BANK_SEL, reg | BANK_SEL_SENSOR);
...
int Maixduino_GC0328::reverse_u32pixel(uint32_t* addr,uint32_t length)
{
  if(NULL == addr)
    return -1;
...
int Maixduino_GC0328::sensor_snapshot( )
{	
    //wait for new frame
    g_dvp_finish_flag = 0;
